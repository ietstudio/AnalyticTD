...
- (void)awakeFromNib {
    ...
    [self.typePickerData addObject:@"匿名帐户"];
    [self.typePickerData addObject:@"显性注册帐户"];
    [self.typePickerData addObject:@"新浪微博"];
    [self.typePickerData addObject:@"QQ帐户"];
    [self.typePickerData addObject:@"腾讯微博"];
    [self.typePickerData addObject:@"91帐户"];
...
- (IBAction)setAccountID {
    self.tdgaAccount = [TDGAAccount setAccount:self.accountIDField.text];
}

- (IBAction)setAccountName {
    [self.tdgaAccount setAccountName:self.accountNameField.text];
}
...
- (IBAction)setAccountType {
    ...
    [self.tdgaAccount setAccountType:(TDGAAccountType)row];
}

- (IBAction)setLevel {
    [self.tdgaAccount setLevel:[self.levelField.text intValue]];
}

- (IBAction)setGender {
    [self.tdgaAccount setGender:(TDGAGender)[self.genderPickerView selectedRowInComponent:0]];
}

- (IBAction)setAge {
    [self.tdgaAccount setAge:[self.ageField.text intValue]];
}

- (IBAction)setGameServer {
    [self.tdgaAccount setGameServer:self.gameServerField.text];
}
